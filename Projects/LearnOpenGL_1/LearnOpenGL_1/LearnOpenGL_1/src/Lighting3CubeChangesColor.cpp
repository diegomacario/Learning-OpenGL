...
int main()
{
   ...
   Shader lightingShader("shader/3.1.materials.vs", "shader/3.1.materials.fs");
   Shader lampShader("shader/3.1.lamp.vs", "shader/3.1.lamp.fs");

   // Initialize the relevant buffers with the data that we wish to render
   // ****************************************************************************************************
   //                     Positions            Normals
   //                    <--------------->    <--------------->
   float vertices[] = { -0.5f, -0.5f, -0.5f,  0.0f,  0.0f, -1.0f,
   ...
   // Connect the vertex positions that are inside VBO with attribute 0 of the vertex shader
   // The vertex shader of the light source does not use the normals, so we do not connect them with an attribute
   glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 6 * sizeof(float), (void*)0);
   glEnableVertexAttribArray(0);
   ...
   while(!glfwWindowShouldClose(window))
   {
      ...
      glm::vec3 diffuseColor = lightColor   * glm::vec3(0.5f); // Decrease the influence of the diffuse component
      glm::vec3 ambientColor = diffuseColor * glm::vec3(0.2f); // Give the ambient component a low influence
