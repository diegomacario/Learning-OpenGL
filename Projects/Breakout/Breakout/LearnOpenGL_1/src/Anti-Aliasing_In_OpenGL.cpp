...
int main()
{
    ...
    // Hint that we want to use a multisampled buffer with 4 samples per pixel
    ...
    // Enabled by default on some drivers, but not on all, so enable it to make sure that the code works with any drivers
    glEnable(GL_MULTISAMPLE);

    // build and compile shaders
    // -------------------------

    // Note: The commented line below is needed if you want to launch this application by double-clicking LearnOpenGL_1.exe
    //       To launch it by pressing F5 in Visual Studio, the uncommented line below is needed.
