...
int main()
{
    ...
    // -------------------------
    Shader shaderRed("shader/14.1.uniform_buffer_objects.vs", "shader/14.1.uniform_buffer_objects_red.fs");
    Shader shaderGreen("shader/14.1.uniform_buffer_objects.vs", "shader/14.1.uniform_buffer_objects_green.fs");
    Shader shaderBlue("shader/14.1.uniform_buffer_objects.vs", "shader/14.1.uniform_buffer_objects_blue.fs");
    Shader shaderYellow("shader/14.1.uniform_buffer_objects.vs", "shader/14.1.uniform_buffer_objects_yellow.fs");
    ...
    glBufferData(GL_UNIFORM_BUFFER, 2 * sizeof(glm::mat4), NULL, GL_STATIC_DRAW); // reserve space for two matrices
    glBindBuffer(GL_UNIFORM_BUFFER, 0);

    // link the uniform buffer to binding point 0
    glBindBufferRange(GL_UNIFORM_BUFFER, 0, uboMatrices, 0, 2 * sizeof(glm::mat4));

    // store the projection matrix in the uniform buffer object
    // we only do this once now because we're not using zoom anymore by changing the FoV
    glm::mat4 projection = glm::perspective(45.0f, (float)SCR_WIDTH / (float)SCR_HEIGHT, 0.1f, 100.0f);
    glBindBuffer(GL_UNIFORM_BUFFER, uboMatrices);
    glBufferSubData(GL_UNIFORM_BUFFER,
                    0,                           // Offset
                    sizeof(glm::mat4),           // Size of data
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        // store the view matrix in the uniform buffer object
        // we only have to do this once per loop iteration instead of 4 times (once per shader program)
        glm::mat4 view = camera.GetViewMatrix();
        glBindBuffer(GL_UNIFORM_BUFFER, uboMatrices);
        glBufferSubData(GL_UNIFORM_BUFFER,
                        sizeof(glm::mat4),     // Offset
                        sizeof(glm::mat4),     // Size of data
                        glm::value_ptr(view)); // Data
        glBindBuffer(GL_UNIFORM_BUFFER, 0);
