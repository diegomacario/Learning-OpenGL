...
// 1) Create memory on the GPU where we can store the vertex data
//       - We manage this memory through Vertex Buffer Objects (VBOs). A VBO can store a large number of vertices in the GPU's memory.
...
// You cannot set one of the mipmap filtering options as the magnification filter.
// Mipmaps are used when textures get downscaled.
// Trying to use one of the mipmap filtering options as the magnification filter results in the GL_INVALID_ENUM error code.
...
// The location of a texture is known as its texture unit.
...
// The advantage of transforming an object's vertices to several intermediate coordinate systems is that
...
// 3) View space:
// Also known as the camera space or the eye space
...
// vec3 reflectDir = reflect(-lightDir, norm); // The reflect function expects the light direction to point from
