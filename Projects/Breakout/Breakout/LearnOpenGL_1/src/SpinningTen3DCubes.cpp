...
int main()
{
   ...
   // There is no need to calculate it for every frame since it rarely changes
   glm::mat4 projection;
   projection = glm::perspective(glm::radians(45.0f),                    // FoV
                                 (float) SCR_WIDTH / (float) SCR_HEIGHT, // Aspect ratio
                                 0.1f,                                   // Near
                                 100.0f);                                // Far

   // Get the locations of the projection matrix in the vertex shader
   ...
   while(!glfwWindowShouldClose(window))
   {
      ...
      view = glm::lookAt(glm::vec3(camX, 0, camZ),     // From
                         glm::vec3(0.0f, 0.0f, 0.0f),  // At
                         glm::vec3(0.0f, 1.0f, 0.0f)); // Up

      // Get the location of the view matrix in the vertex shader
      ...
      for (unsigned int i = 0; i < 10; ++i)
      {
         ...
         // Get the location of the model matrix in the vertex shader
