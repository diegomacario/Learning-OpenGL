...
int main()
{
   ...
   Shader lightingShader("shader/4.3.lighting_maps.vs", "shader/4.3.lighting_maps.fs");
   Shader lampShader("shader/4.3.lamp.vs", "shader/4.3.lamp.fs");
   ...
   unsigned int emissionMap = loadTexture("tex/matrix.jpg");
   // The loadTexture function does not unbind the texture after it is loaded, so we do it here
   glBindTexture(GL_TEXTURE_2D, 0);
   ...
   while(!glfwWindowShouldClose(window))
   {
      ...
      // Light properties
      lightingShader.setVec3("light.position", rotatedLightPos);
